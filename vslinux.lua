...
local function vsver()
    local ver = tonumber(string.match(_ACTION or "", "vs(%d*)")) or 0
    return ver
end
...
p.override(p.vstudio, "archFromConfig", function(oldfn, cfg, force)
    if vsver() >= 2015 and cfg.system == p.LINUX then
...
p.override(p.vstudio.vc2010.elements, "project", function(oldfn, prj)
    ...
    if vsver() >= 2015 and prj.system == p.LINUX then
...
p.override(p.vstudio.vc2010, "characterSet", function(oldfn, cfg)
    if not (vsver() >= 2015 and cfg.system == p.LINUX) then
        oldfn(cfg)
    end
end)
...
function m.languageStandard(cfg)
    if vsver() >= 2015 and cfg.system == p.LINUX then
        if cfg.cppdialect == "C++latest" then
            p.vstudio.vc2010.element("CppLanguageStandard", nil, "c++17")
...
p.override(p.vstudio.vc2010, "warningLevel", function(oldfn, cfg)
    if vsver() >= 2015 and cfg.system == p.LINUX and cfg.warnings and cfg.warnings ~= "Off" then
        p.vstudio.vc2010.element("WarningLevel", nil, "EnableAllWarnings")
    elseif (vsver() >= 2015 and cfg.system == p.LINUX and cfg.warnings) or not (vsver() >= 2015 and cfg.system == p.LINUX) then
        oldfn(cfg)
    end
end)
